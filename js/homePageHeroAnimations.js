export function initHomePageHeroAnimations() {
  ...
  if (finleyHeading) {
    ...
    if (statsContainer) {
      ...
      const statsActualStartTime =
        clubNameAnimationStartTime + clubNameAnimationDuration + 200; // 200ms buffer
      ...
      setTimeout(() => {
        elementsToRender.forEach((element, index) => {
          ...
          if (element.type === "stat") {
            ...
            if (stat.label === "AUSGEZEICHNET")
              ratingText.classList.add("label-ausgezeichnet");
            else if (stat.label === "SEHR GUT")
              ratingText.classList.add("label-sehr-gut");
        ...
        setTimeout(() => {
          ...
          const charlierText = document.querySelector(
            ".card-bottom h1.font-24"
          );
          const finleyHeading = document.querySelector(
            ".finley-animated-heading"
          );
          // Club text already selected as clubText

          // Add animation to move text up and out - synchronized
          if (charlierText) {
            charlierText.style.transition =
              "transform 1.2s ease-in-out, opacity 1s ease-in-out";
          ...
          if (clubText) {
            ...
            setTimeout(() => {
              ...
              const overlay2 = document.querySelector(
                ".card-overlay.bg-gradient.fa-rotate-180"
              );
      ...
      console.warn(
        "HeroAnimations: Statistics container .statistics-container not found."
      );
    }
  } else {
    console.warn(
      "HeroAnimations: Finley heading .finley-animated-heading not found."
    );
  }
}
