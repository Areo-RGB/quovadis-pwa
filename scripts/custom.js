      ...
      selectField.forEach(el =>
        el.addEventListener('change', e => {
          if (el.value !== 'default') {
            ...
            if (validEl) validEl.classList.remove('disabled');
            var invalidEl = el.parentElement.querySelectorAll('.invalid, em, span')[0];
            if (invalidEl) invalidEl.classList.add('disabled');
          }
          if (el.value == 'default') {
            var spanEl = el.parentElement.querySelectorAll('span, .valid, em')[0];
            if (spanEl) spanEl.classList.add('disabled');
            var invalidEl2 = el.parentElement.querySelectorAll('.invalid')[0];
            if (invalidEl2) invalidEl2.classList.remove('disabled');
            el.parentElement.classList.add('input-style-active');
          }
        })
      );
        ...
        el.addEventListener('click', e => {
          ...
          var timerAdFunction = setInterval(function () {
            if (timedAdTimer <= 1) {
              ...
              document.getElementById(timedAdData).querySelectorAll('span')[0].style.display =
                'none';
            } else {
              //console.log(timedAdTimer);
            }
            document.getElementById(timedAdData).querySelectorAll('span')[0].innerHTML =
              timedAdTimer -= 1;
          }, 1000);
        })
      );
      ...
      var timerAdFunction = setInterval(function () {
        if (autoAdTime <= 1) {
          ...
          var downloadTimer = setInterval(function () {
            ...
            document.getElementById(autoAdId).querySelectorAll('span')[0].innerHTML =
              autoAdCloseTime -= 1;
          }, 1000);
        }
        autoAdTime -= 1;
    ...
    async getStorageInfo() {
      if ('storage' in navigator && 'estimate' in navigator.storage) {
        ...
        const info = {
          ...
          percentage: estimate.usage ? estimate.usage / (estimate.quota || 1) : 0,
          percentageText:
            Math.round((estimate.usage ? estimate.usage / (estimate.quota || 1) : 0) * 100) + '%',
        };
