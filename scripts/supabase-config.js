 ...
 * QuoVadis Project - YoYo Fitness Test Integration
 */
...
function initializeSupabase() {
  if (typeof supabase !== 'undefined' && supabase.createClient) {
    ...
    console.error(
      'Supabase SDK not loaded. Make sure to include the Supabase script before this file.'
    );
    return null;
  }
}
...
const YoYoDatabase = {
  ...
  async saveAthleteData(athleteName, distance, warningState, sessionName = 'Default Session') {
    ...
    try {
      const { data, error } = await supabaseClient.from('yoyo_sessions').upsert(
        {
      ...
      console.log(
        'Athlete data saved successfully:',
        athleteName,
        distance + 'm',
        'State:',
        warningState
  ...
  async resetAllData() {
    ...
    try {
      const { error } = await supabaseClient.from('yoyo_sessions').delete().neq('id', 0); // This will delete all rows since id is never 0
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { SUPABASE_CONFIG, YoYoDatabase, initializeSupabase };
}
